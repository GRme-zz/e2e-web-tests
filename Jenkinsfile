  ...
  stages {
    stage('Test') {
      steps {
        ...
        sh "${params.sudo} ${params.docker} exec -i e2e-web-tests-docker-container bash -c \"cd /my_tests && (xvfb-run --server-args=\'-screen 0 1600x1200x24\' npm run ${params.run_script_method} -- ${params.browser} ${params.tags} || true) && google-chrome --version && firefox --version\""
